...
fn main() {
    ...
    let mut stack: Vec<(Vec<String>, NodeIndex)> = Vec::new();    

    stack.push((Vec::new(),start_index));
    while let Some((mut current_path, ni)) = stack.pop() {
        ...
        if val == "end" {
            ...
            for neighbor in neighbors { 
                ...
                if nv != "start" &&  nv.to_uppercase() == nv || !current_path.contains(&nv){
                        stack.push((current_path.clone(), neighbor));
                }
            }
        }
    }
